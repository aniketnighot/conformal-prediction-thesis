...
class WRDSOptionsDataset(OptionsDataset):
    ...
    * ``am_settlement`` (to be able to discard weekly options)
    ...
    def load_data(cls) -> pd.DataFrame:
        ...
        Notes
        -----
        When multiple CSV files exist, behavior is controlled by OPDS_FILE_MODE environment variable:
        - 'latest' (default): Use most recently modified file only
        - 'combined': Load and concatenate all CSV files into a single dataset
        """

        data_dir = Path(os.environ['OPDS_WRDS_DATA_DIR'])
        file_mode = os.getenv('OPDS_FILE_MODE', 'latest').lower()
        ...
        if len(csv_files) == 0:
            ...
            if file_mode == 'combined':
                ...
                print(f"Multiple CSV files found. Loading all {len(csv_files)} files (mode: {file_mode})")
                dataframes = []
                ...
                print(f"Multiple CSV files found. Using latest: {latest_file} (mode: {file_mode})")
        else:
