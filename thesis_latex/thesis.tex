...
%%%%%% Algorithmen-Umgebung (fuer Algorithm 1 in der Methodology):
\usepackage{algorithm}
\usepackage{algpseudocode}

%%%%%% Theorem-Umgebung (fuer Theorem 2.1 in der Methodology):
\usepackage{amsthm}
...
\else
\selectlanguage{ngerman} % ngerman, british
\fi
...
\input{TeXFiles/004_Acknowledgement}
\tableofcontents
...
\input{TeXFiles/040_LiteratureReview}
\input{TeXFiles/050_Methodology}
\input{TeXFiles/060_Implementation}
\input{TeXFiles/070_ExperimentsResults}

\input{TeXFiles/080_AnalysisDiscussion}
\input{TeXFiles/090_Conclusion}
...
%%\bibliographystyle{alpha} % Initials first author and year

% Wenn man folgenden Stil verwenden will, dann muss
% \usepackage{harvard} vor \begin{document} aktiviert werden:
%\bibliographystyle{agsm} % (Authors in square brackets)
%\newpage
%\clearpage
